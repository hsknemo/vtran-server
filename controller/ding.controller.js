...
const ding_find_user_func = async (req, res) => {
  try {
    ...
    if (data.length) {
      let user = await userModel.findUserPool(
        data.map((item) => item.fromUser),
      );
      data.forEach((item) => {
...
const ding_unread_count_func = async (req, res) => {
  try {
    let userId = req.tokenResolveResult.id;
    let count = await dingModel.getUnreadDingCount(userId);
...
const ding_unread_count = {
  method: "get",
  path: `${routeName}/unread-count`,
  midFun: [AUTHORIZATION],
  func: ding_unread_count_func,
  desc: "获取未读叮消息数",
};

module.exports = [ding_add, ding_find_user, ding_delete, ding_unread_count];
